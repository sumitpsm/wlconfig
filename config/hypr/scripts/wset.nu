...
# Function to set the wallpaper with awww
def set_theme [file: path] {
  let fps = 60
  let type = if ((random int) mod 5) != 0 { "any" } else { "fade" }
  let duration = 1.5
  let bezier = ".43,1.19,1,.4"
  awww img $file --transition-fps $fps --transition-type $type --transition-duration $duration --transition-bezier $bezier
}
...
def main [file?: string] {
  if $file != null and ($file | path type) == "file" {
    ...
    #   let cache_file = $"($env.HOME)/.cache/awww/($current_monitor)"
    ...
    #     sleep 1sec
    #     awww img $wallpaper_path --transition-type fade --transition-duration 2
    #   }
    # }
  }
}
